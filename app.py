...
# ── NEW: persistent session helpers ──
def save_session(username):
    token = secrets.token_hex(32)
    with open(SESSION_FILE, "wb") as f:
        pickle.dump({"username": username, "token": token}, f)
...
# ── NEW: restore login from saved session file on startup ──
if not st.session_state.logged_in:
    _saved = load_session()
    if _saved and _saved.get("username") in st.session_state.users:
...
def show_login():
    ...
    if st.button("Sign In →", key="li_btn"):
        ...
        if udata and udata.get("password") == p:
            ...
            save_session(u)   # ── NEW: persist login to disk ──
...
def show_sidebar():
    ...
    if st.sidebar.button("🚪 Sign Out", key="signout"):
        clear_session()   # ── NEW: delete session file from disk ──
