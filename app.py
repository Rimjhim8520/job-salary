...
.landing-cta .stButton>button{{
  background:linear-gradient(135deg,#fff,#e8e0ff)!important;
...
# =========================
def show_landing():
    st.markdown('<div style="padding:28px 28px 0;">', unsafe_allow_html=True)

    # Hero banner
    st.markdown(f"""
    <div style="background:{HERO_BG};border-radius:24px;padding:64px 40px 56px;
                text-align:center;position:relative;overflow:hidden;
                box-shadow:0 16px 48px rgba(99,102,241,0.35);margin-bottom:28px;">
      ...
      <div style="position:absolute;top:30px;left:60px;width:80px;height:80px;
      ...
      <div style="position:relative;z-index:1;">
        ...
        <div style="font-family:'Plus Jakarta Sans',sans-serif;font-size:52px;font-weight:900;
                    color:#fff;line-height:1.1;margin-bottom:20px;letter-spacing:-1px;">
          Know Your<br><em style="font-style:normal;opacity:0.88;">True Market Worth</em>
        </div>
        <div style="font-size:16px;color:rgba(255,255,255,0.78);line-height:1.8;
                    max-width:560px;margin:0 auto 32px;">
          Predict your salary instantly using Machine Learning — based on your
          experience, education, skills, certifications, and industry.
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-bottom:10px;">
          <span style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);
                       color:#fff!important;padding:8px 20px;border-radius:99px;font-size:13px;font-weight:600;">
            ✅ 95% Accuracy
          </span>
          <span style="background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);
                       color:#fff!important;padding:8px 20px;border-radius:99px;font-size:13px;font-weight:600;">
    ...
    f1, f2, f3, f4 = st.columns(4)
    cards = [
        ("🎯", "Accurate Predictions",  "KNN model trained on 250K+ real salary records across industries."),
        ("⚡", "Instant Results",        "Get your salary prediction in under 1 second — no waiting."),
        ("📊", "Full Career Analytics",  "Dashboard, roadmap, benchmarks and industry trend reports."),
        ("📱", "WhatsApp Export",        "Send your complete career insights report directly to WhatsApp."),
    ]
    for col, (icon, title, desc) in zip([f1, f2, f3, f4], cards):
        with col:
            st.markdown(
    ...
    # How it works
    st.markdown(f'<br><div class="section-heading" style="padding:0;">🔄 How It Works</div>', unsafe_allow_html=True)
    h1, h2, h3, h4 = st.columns(4)
    steps = [
        ("1️⃣", "Fill Profile",      "Enter your experience, skills, education, and job details"),
        ("2️⃣", "Run Prediction",    "Our KNN model instantly calculates your market salary"),
        ("3️⃣", "Get Insights",      "See career roadmap, benchmarks, and growth tips"),
        ("4️⃣", "Share via WhatsApp","Send your full report to yourself on WhatsApp"),
    ]
    for col, (num, title, desc) in zip([h1, h2, h3, h4], steps):
        with col:
            st.markdown(
                ...
                f'<div style="font-size:28px;margin-bottom:10px;">{num}</div>'
                f'<div style="font-size:14px;font-weight:700;color:{TEXT1};margin-bottom:6px;">{title}</div>'
                f'<div style="font-size:12px;color:{TEXT2};line-height:1.5;">{desc}</div></div>',
                unsafe_allow_html=True
            )
    ...
    with cta_col:
        ...
        if st.button("🚀  Get Started — It's Free", key="landing_cta"):
            st.session_state.show_landing = False
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

    st.markdown(
        f'<div style="text-align:center;font-size:12px;color:{TEXT3};margin-top:10px;padding-bottom:20px;">'
        f'Already have an account? Click Get Started to sign in.</div>',
        unsafe_allow_html=True
    )
    st.markdown('</div>', unsafe_allow_html=True)
...
def show_topbar():
    ...
    # NOTE: "home" tab is removed — tabs start with predict
...
def show_insights():
    ...
    sc     = inp["skills_count"]; cert = inp["certifications"]
    ...
    <div class="wa-card">
      <div class="wa-title">📱 Send Insights to WhatsApp</div>
      <div class="wa-desc">
        Get your complete career insights report — salary prediction, growth tips, skill roadmap,
        and industry trends — delivered directly to your WhatsApp.
      </div>
    </div>""", unsafe_allow_html=True)
    ...
    with wa_col1:
        wa_number = st.text_input(
            ...
            placeholder="e.g. 919876543210  (91 for India)",
            key="wa_number",
            help="Enter your number with country code, no + or spaces."
        )
    with wa_col2:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        if st.button("📤 Generate WhatsApp Link", key="wa_btn"):
...
def show_compare():
    ...
    top25  = int(salary*1.22); top10 = int(salary*1.48); top5 = int(salary*1.75); mx = top5
    bm = [
        ("Entry Level (0–2 yrs)", int(base*0.6),  "#94a3b8"),
        ("Mid Level (3–6 yrs)",   int(base*0.85), "#0ea5e9"),
        ("📍 Your Salary",         salary,          ACCENT),
        ("Top 25%",               top25,           ACCENT2),
        ("Top 10%",               top10,           "#7c3aed"),
        ("Top 5% — Elite",        top5,            "#9333ea"),
    ]
    c1, c2 = st.columns([3,2], gap="large")
    with c1:
        st.markdown(f'<div class="card"><div class="card-title">📊 Market Benchmarks — {job}</div>', unsafe_allow_html=True)
...
# =========================
if not st.session_state.logged_in:
    # Theme toggle in sidebar always available
    st.sidebar.markdown('<div class="theme-sb" style="padding:16px 14px 8px;">', unsafe_allow_html=True)
    if st.sidebar.button(TOGGLE_LBL, key="auth_theme"):
        st.session_state.dark_mode = not st.session_state.dark_mode; st.rerun()
    st.sidebar.markdown('</div>', unsafe_allow_html=True)
    ...
    # Top brand bar
    st.markdown(
        f'<div style="background:{HERO_BG};padding:18px 28px;border-radius:16px;'
        f'text-align:center;font-family:\'Plus Jakarta Sans\',sans-serif;font-size:26px;'
        f'font-weight:900;color:#fff;margin-bottom:24px;'
        f'box-shadow:0 8px 32px rgba(99,102,241,0.3);">💼 SalaryIQ — Know Your Worth</div>',
        unsafe_allow_html=True
    )

    # ── LANDING shown first; login/signup only after "Get Started" ──
    ...
    tab = st.session_state.active_tab
    # Redirect any leftover "home" tab reference to predict
    if tab == "home":
        st.session_state.active_tab = "predict"
        tab = "predict"
